...
case "$HOST" in
    localads.chengzijianzhan.cn)
        ...
        ;;
    www.life-data.cn)
        [[ "$IDENTITY_KEY" == "groupid" && ("$IDENTITY_VALUE" =~ ^[0-9]+$ || "$IDENTITY_VALUE" == "auto") ]] || { echo "ERROR: life-data requires numeric groupid or auto" >&2; exit 2; }
        ;;
...
tell application "Google Chrome"
    ...
    repeat with w from 1 to count of windows
        repeat with t from 1 to count of tabs of window w
            ...
            if tabURL contains "$HOST$PATH_CONTAINS" and ("$IDENTITY_VALUE" is "auto" or tabURL contains "$IDENTITY_KEY=$IDENTITY_VALUE") then
